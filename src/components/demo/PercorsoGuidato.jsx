...
export default function PercorsoGuidato({ chiave, titolo, passi }) {
  ...
  if (!aperto || !passi?.length) {
    return (
      <button
        ...
        data-comando
        className="fixed bottom-4 right-4 z-[60] flex h-11 items-center gap-2 rounded-full bg-ink-900 px-4 text-piccolo font-semibold text-white shadow-[var(--shadow-lift)] hover:bg-ink-800"
      >
        <Icona misura="sm" nome="Compass" className="text-sole-400" />
        Percorso guidato
      </button>
    );
  }
  ...
  return (
    <div className="pointer-events-none fixed inset-x-0 bottom-0 z-[60] flex justify-center p-3 sm:inset-x-auto sm:right-4 sm:justify-end sm:p-4">
      <div className="pointer-events-auto w-full max-w-md overflow-hidden rounded-[var(--radius-scheda)] bg-ink-900 shadow-[var(--shadow-lift)]">
        {/* avanzamento */}
          ...
          {passi.map((p, i) => (
            <button
              ...
              className={`h-1.5 flex-1 rounded-full transition-colors ${
                i <= indice ? "bg-sole-400" : "bg-white/15 hover:bg-white/30"
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex items-start gap-3 p-4">
          <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-sole-500 text-corrente font-extrabold text-ink-900">
            {indice + 1}
          </span>

          <div className="min-w-0 flex-1">
            <p className="text-mini font-semibold uppercase tracking-wide text-sole-300">
              {titolo}
            </p>
            <h2 className="mt-1 text-testo font-bold text-white">{passo.titolo}</h2>
            <p className="mt-1 text-piccolo leading-relaxed text-white/70">{passo.testo}</p>

            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Link
                href={passo.dove}
                data-comando
                className="flex h-9 items-center gap-1.5 rounded-[var(--radius-controllo)] bg-sole-500 px-3 text-piccolo font-semibold text-ink-900 hover:bg-sole-400"
              >
                Portami qui
              ...
              {indice > 0 ? (
                <button
                  ...
                  data-comando
                  className="flex h-9 items-center rounded-[var(--radius-controllo)] px-3 text-piccolo font-medium text-white/70 hover:bg-white/10 hover:text-white"
                >
                  Indietro
                </button>
              ) : null}
          ...
          <div className="flex shrink-0 gap-1">
            <button
              ...
              className="flex size-8 items-center justify-center rounded-[var(--radius-controllo)] text-white/50 hover:bg-white/10 hover:text-white"
            >
