...
export default function GestionaleCard({ gestionale, categoria, base }) {
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-line bg-white transition-all hover:-translate-y-0.5 hover:border-brand-200 hover:shadow-[var(--shadow-lift)]">
      ...
      >
        <div className="absolute inset-0 flex items-center gap-2 p-4 opacity-70">
          <div className="flex h-full w-9 flex-col gap-1.5 rounded-lg bg-white/80 p-1.5">
          ...
          <div className="flex h-full flex-1 flex-col gap-1.5">
            <div className="flex gap-1.5">
              <span className="h-6 flex-1 rounded-md bg-white/85" />
              <span className="h-6 flex-1 rounded-md bg-white/85" />
              <span className="h-6 flex-1 rounded-md bg-white/85" />
            </div>
            <div className="flex-1 rounded-md bg-white/85" />
          </div>
        </div>
        {categoria ? (
          <span
            className="absolute right-3 top-3 flex size-9 items-center justify-center rounded-lg bg-white shadow-[var(--shadow-soft)]"
            style={{ color: `var(--cat-${categoria.colore})` }}
          >
            <Icona nome={categoria.icona} className="size-[18px]" />
          </span>
        ) : null}
      </div>
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <Pastiglia variante="contorno">
            <Icona nome="Settings2" className="size-3" />
            Personalizzabile
          </Pastiglia>
        </div>
        ...
        <p className="mt-2 line-clamp-2 text-[14px] leading-relaxed text-ink-500">{sottotitolo}</p>
        ...
        <ul className="mt-4 flex flex-wrap gap-1.5">
          ...
          {funzioni.length > 5 ? (
            <li className="px-1 py-1 text-[11.5px] font-medium text-ink-400">
              +{funzioni.length - 5}
            </li>
          ) : null}
        </ul>
        ...
        <div className="mt-5 flex items-center justify-between border-t border-line-soft pt-4">
          ...
          {demoDisponibile ? (
            <span className="inline-flex items-center gap-1.5 text-[12.5px] font-medium text-emerald-700">
              <Icona nome="PlayCircle" className="size-3.5" />
              Demo disponibile
            </span>
          ) : base ? (
            <span className="text-[12px] text-ink-400">Base {base.nome}</span>
          ) : null}
        </div>
      </div>
    </article>
